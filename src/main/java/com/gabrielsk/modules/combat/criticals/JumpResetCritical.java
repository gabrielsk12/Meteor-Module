package com.gabrielsk.modules.combat.criticals;

import net.minecraft.client.MinecraftClient;
import net.minecraft.entity.Entity;
import net.minecraft.network.packet.c2s.play.PlayerMoveC2SPacket;
import net.minecraft.util.math.Vec3d;

/**
 * Jump reset critical exploit
 * Exploits jump mechanics by resetting jump state
 * Very effective and hard to detect
 */
public class JumpResetCritical implements CriticalStrategy {
    
    private long lastExecution = 0;
    private static final long MIN_DELAY = 75;
    private int jumpPhase = 0;
    
    @Override
    public boolean execute(MinecraftClient mc, Entity target) {
        if (mc.player == null || mc.getNetworkHandler() == null) return false;
        
        try {
            Vec3d currentVel = mc.player.getVelocity();
            double x = mc.player.getX();
            double y = mc.player.getY();
            double z = mc.player.getZ();
            
            // Phase 1: Small upward velocity
            if (jumpPhase == 0) {
                mc.player.setVelocity(currentVel.x, 0.1, currentVel.z);
                mc.player.velocityModified = true;
                jumpPhase = 1;
            }
            // Phase 2: Send ground packet to reset
            else if (jumpPhase == 1) {
                mc.getNetworkHandler().sendPacket(
                    new PlayerMoveC2SPacket.PositionAndOnGround(x, y + 0.001, z, true)
                );
                jumpPhase = 2;
            }
            // Phase 3: Complete critical
            else {
                mc.getNetworkHandler().sendPacket(
                    new PlayerMoveC2SPacket.PositionAndOnGround(x, y, z, false)
                );
                jumpPhase = 0;
                lastExecution = System.currentTimeMillis();
                return true;
            }
            
            return false;
        } catch (Exception e) {
            jumpPhase = 0;
            return false;
        }
    }
    
    @Override
    public boolean canExecute(MinecraftClient mc) {
        if (mc.player == null) return false;
        
        // Only check on phase 0 (new attempt)
        if (jumpPhase != 0) return true;
        
        // Check basic conditions
        if (!mc.player.isOnGround()) return false;
        if (mc.player.isTouchingWater()) return false;
        if (mc.player.isInLava()) return false;
        if (mc.player.hasVehicle()) return false;
        if (mc.player.isClimbing()) return false;
        
        // Anti-spam delay
        if (System.currentTimeMillis() - lastExecution < MIN_DELAY) return false;
        
        return true;
    }
    
    @Override
    public String getName() {
        return "Jump Reset";
    }
    
    @Override
    public double getDetectionRisk() {
        return 0.3; // Medium-low risk
    }
    
    @Override
    public double getDamageMultiplier() {
        return 1.5; // Standard critical
    }
    
    @Override
    public int getExecutionSpeed() {
        return 10; // Fast
    }
}
